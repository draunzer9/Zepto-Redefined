...
export class SmartBasketEngine {
  ...
  public static getRecommendations(
    ...
    for (const product of PRODUCTS) {
      ...
      let reason = 'Based on your previous orders.';
      if (replenishmentDueScore >= 25) {
        reason = `Usually purchased every ${Math.round(medianInterval)} days.`;
      } else if (frequencyScore >= 20) {
        reason = `Purchased in ${purchaseCount} of your recent orders.`;
      } else if (isSaturdayPattern && config.simulatedDayOfWeek === 'Saturday') {
        reason = 'Often purchased on weekends.';
      } else if (daysSinceLastPurchase > maxDueDays) {
        reason = "You're likely running low based on your usual purchase cycle.";
      }
